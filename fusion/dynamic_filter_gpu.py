...
import cv2, argparse, time, torch
import numpy as np
...
def filter(dataset_root, scan, img_folder, cam_folder,
    ...
    for ref_view, src_views in pairs:
        ...
        geo_mask = geo_mask >=3    # condition filter
...
if __name__=="__main__":
    ...
    parser.add_argument("-e",'--eval_folder', default="/root/autodl-tmp/cascade-mvsnet-master/CasMVSNet/outputs", type=str, help='eval output location')#"../../outputs"
    parser.add_argument("-r",'--root_folder', default="/root/autodl-tmp", type=str, help='dataset root location')
    ...
    parser.add_argument('-o', '--outply_folder', default="/root/autodl-tmp/cascade-mvsnet-master/CasMVSNet/outputs", type=str, help='dtu or tanks')
    ...
    if args.dataset == "dtu":
        ...
        dataset_root = os.path.join(root_dir, "tank", tanks_group, )
        if tanks_group == "intermediate":
            #scans = ['Family','Francis',  , ]#condition>5
           # scans = ['Francis','Train','Panther','Lighthouse','Horse','M60']
            scans = ['Panther']
