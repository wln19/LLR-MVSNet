...
parser.add_argument('--datafile', default='tank', help='tank or dtu')
...
parser.add_argument('--cost_aggregation', type=str, default="91", help='cost_aggregation')
...
if __name__ == '__main__':
    ...
    # model, optimizer
    model=LLSRMVSNet(datafile=args.datafile,refine=False, ndepths=[int(nd) for nd in args.ndepths.split(",") if nd],
                          depth_interals_ratio=[float(d_i) for d_i in args.depth_inter_r.split(",") if d_i],
                          share_cr=args.share_cr,
                          cr_base_chs=[int(ch) for ch in args.cr_base_chs.split(",") if ch],
                          grad_method=args.grad_method,cost_aggregation=args.cost_aggregation,mode=args.mode)
    model.to(device)
    model_loss =LLSR_mvsnet_loss
    ...
    train_dataset = MVSDataset(args.trainpath, args.trainlist, "train", 3, args.numdepth, args.interval_scale,robust_train=True)
    test_dataset = MVSDataset(args.testpath, args.testlist, "test", 5, args.numdepth, args.interval_scale,robust_train=False)
    ...
    if is_distributed:
        ...
        TestImgLoader = DataLoader(test_dataset, args.batch_size, sampler=test_sampler, num_workers=1, drop_last=True,
                                   pin_memory=args.pin_m)
    else:
        TrainImgLoader = DataLoader(train_dataset, args.batch_size, shuffle=True, num_workers=1, drop_last=True,
                                    pin_memory=args.pin_m)
        TestImgLoader = DataLoader(test_dataset, args.batch_size, shuffle=False, num_workers=1, drop_last=True,
                                   pin_memory=args.pin_m)
