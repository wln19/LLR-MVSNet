...
from models import *
from utils import *
from datasets.data_io import read_pfm, save_pfm, read_pairfile, read_img, read_cam_file, save_mask, bilinear_sampler
from plyfile import PlyData, PlyElement
from PIL import Image
from fusion.gipuma import gipuma_filter
...
parser.add_argument('--mode', default='test', help=' test', choices=['test'])
parser.add_argument('--dataset', default='dtu_yao_eval', help='select dataset')
parser.add_argument('--datafile', default='tanks', type=str, help='dtu or tanks')
...
#filter by gimupa
parser.add_argument('--fusibile_exe_path', type=str, default='/media/yons/10T1/wanglina/fusibile/fusibile/fusibile')
...
def save_scene_depth(testlist):
    ...
    test_dataset = MVSDataset(args.testpath, testlist, "test", args.num_view, args.numdepth, Interval_Scale,
                              max_h=args.max_h, max_w=args.max_w, fix_res=args.fix_res,robust_train=True)
    ...
    model = LLSRMVSNet(refine=False,datafile=args.datafile,ndepths=[int(nd) for nd in args.ndepths.split(",") if nd],
                          depth_interals_ratio=[float(d_i) for d_i in args.depth_inter_r.split(",") if d_i],
                          share_cr=args.share_cr,
                          cr_base_chs=[int(ch) for ch in args.cr_base_chs.split(",") if ch],
                          grad_method=args.grad_method,mode=args.mode)
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        for src_view in src_views:
            ...
            #cd geo_mask_sum += geo_mask.astype(np.int32)
...
def pcd_filter_worker(scan):
    if args.testlist != "all":
        #scan_id = int(scan[4:])
        scan_id =scan
        save_name = '{:0>3}.ply'.format(scan_id)
    else:
